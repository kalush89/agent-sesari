...
/**
 * Creates the DynamoDB table for revenue signal events
 */
async function createRevenueSignalsTable(
  tableName: string,
  region: string
): Promise<void> {
  ...
  try {
    ...
    // Create table with eventId primary key and GSI for customer queries
...
async function main() {
  ...
  await createRevenueSignalsTable(tableName, region);
