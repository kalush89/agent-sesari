...
/**
 * Creates or updates the EventBridge rule for daily baseline calculation
 */
async function ensureEventBridgeRule(
  ...
  await client.send(
    new PutRuleCommand({
      Name: ruleName,
      Description: 'Daily trigger for Mixpanel usage baseline calculation and signal detection',
 ...
 */
async function main() {
  const ruleName = process.env.EVENTBRIDGE_RULE_NAME || 'mixpanel-baseline-calculator-daily';
  const functionName = process.env.LAMBDA_FUNCTION_NAME || 'mixpanel-baseline-calculator';
  const scheduleExpression = process.env.SCHEDULE_EXPRESSION || 'cron(0 10 * * ? *)';
  ...
  console.log(`Schedule: ${scheduleExpression} (10 AM UTC daily)`);
  ...
  console.log(`\nThe baseline calculator will run daily at 10 AM UTC.`);
  console.log('\nTo change the schedule, update the SCHEDULE_EXPRESSION environment variable.');
  console.log('Example schedules:');
  console.log('  - cron(0 10 * * ? *) = 10 AM UTC daily');
  console.log('  - cron(0 */12 * * ? *) = Every 12 hours');
