...
async function ensureLambdaRole(
  ...
  const createRoleResponse = await iamClient.send(
    new CreateRoleCommand({
      ...
      Description: 'Execution role for Mixpanel Connector Lambda',
    })
  );
  ...
  await iamClient.send(
    new PutRolePolicyCommand({
      RoleName: roleName,
      PolicyName: 'MixpanelConnectorPolicy',
...
function packageLambda(): Buffer {
  ...
  execSync(
    'zip -r webhook-lambda-package.zip dist node_modules',
    {
      cwd: join(__dirname, '..'),
      stdio: 'inherit',
    }
  ...
  const zipPath = join(__dirname, '..', 'webhook-lambda-package.zip');
...
async function deployLambdaFunction(
  ...
  const environment = {
    Variables: {
      ...
      MIXPANEL_WEBHOOK_SECRET: config.webhookSecret,
  ...
  try {
    ...
    await lambdaClient.send(
      new CreateFunctionCommand({
        ...
        MemorySize: 512,
        Timeout: 10,
        Description: 'Mixpanel webhook processor for behavioral signal detection',
      })
    );
...
async function main() {
  ...
  const roleName = process.env.IAM_ROLE_NAME || 'mixpanel-connector-role';
  ...
  const webhookSecret = process.env.MIXPANEL_WEBHOOK_SECRET;
  const logLevel = process.env.LOG_LEVEL || 'info';

  if (!webhookSecret) {
    throw new Error(
      'MIXPANEL_WEBHOOK_SECRET environment variable is required. Set it before deploying.'
    );
  }
