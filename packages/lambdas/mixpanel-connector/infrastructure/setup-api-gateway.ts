...
async function ensureRestApi(
  ...
  const createResponse = await client.send(
    new CreateRestApiCommand({
      name: apiName,
      description: 'Mixpanel webhook endpoint for behavioral signal detection',
...
/**
 * Creates the /mixpanel-webhook resource path
 */
async function createWebhookResource(
  ...
  // Check if /mixpanel-webhook already exists
  const existingWebhook = resourcesResponse.items?.find(
    (resource) => resource.path === '/mixpanel-webhook'
  );
  if (existingWebhook) {
    console.log(`Resource /mixpanel-webhook already exists: ${existingWebhook.id}`);
  ...
  const createResourceResponse = await client.send(
    new CreateResourceCommand({
      ...
      pathPart: 'mixpanel-webhook',
    })
  );

  console.log(`Created resource /mixpanel-webhook: ${createResourceResponse.id}`);
...
async function setupPostMethod(
  ...
  console.log('Created POST method on /mixpanel-webhook');
...
async function grantApiGatewayPermission(
  ...
  const sourceArn = `arn:aws:execute-api:${region}:${accountId}:${apiId}/*/*/mixpanel-webhook`;
...
async function deployApi(
  ...
  const endpoint = `https://${apiId}.execute-api.${region}.amazonaws.com/${stageName}/mixpanel-webhook`;
 ...
 */
async function main() {
  const apiName = process.env.API_GATEWAY_NAME || 'mixpanel-webhook-api';
  const functionName = process.env.LAMBDA_FUNCTION_NAME || 'mixpanel-connector';
  ...
  // Create /mixpanel-webhook resource
  ...
  console.log('2. Go to Mixpanel Project Settings > Webhooks');
  console.log('3. Add webhook with this URL');
  console.log('4. Configure the webhook signing secret in your Lambda environment variables');
}
