/**
 * Webhook security module for HubSpot signature verification
...
/**
 * Verifies HubSpot webhook signature using HMAC SHA-256
 * 
 * HubSpot sends webhooks with an X-HubSpot-Signature header containing
 * a SHA-256 hash of the request body concatenated with the client secret.
 * 
 * @param payload - Raw webhook payload as string
 * @param signature - HubSpot signature from X-HubSpot-Signature header
 * @param secret - Webhook signing secret from environment
 * @param timestamp - Request timestamp in milliseconds (from X-HubSpot-Request-Timestamp header)
...
export function getWebhookSecret(): string {
  ...
  if (!secret) {
    throw new Error('HUBSPOT_WEBHOOK_SECRET environment variable is not set');
  }
 ...
 * @param eventId - HubSpot event ID if available
