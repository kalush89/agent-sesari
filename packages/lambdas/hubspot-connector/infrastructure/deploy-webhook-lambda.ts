...
async function ensureLambdaRole(
  ...
  const createRoleResponse = await iamClient.send(
    new CreateRoleCommand({
      ...
      Description: 'Execution role for HubSpot Connector Lambda',
    })
  );
  ...
  await iamClient.send(
    new PutRolePolicyCommand({
      RoleName: roleName,
      PolicyName: 'HubSpotConnectorPolicy',
...
async function deployLambdaFunction(
  ...
  const environment = {
    Variables: {
      ...
      HUBSPOT_WEBHOOK_SECRET: config.webhookSecret,
  ...
  try {
    ...
    await lambdaClient.send(
      new CreateFunctionCommand({
        ...
        MemorySize: 512,
        Timeout: 10,
        Description: 'HubSpot webhook processor for relationship signal detection',
      })
    );
...
async function main() {
  ...
  const roleName = process.env.IAM_ROLE_NAME || 'hubspot-connector-role';
  ...
  if (!webhookSecret) {
    throw new Error(
      'HUBSPOT_WEBHOOK_SECRET environment variable is required. Set it before deploying.'
    );
  }
